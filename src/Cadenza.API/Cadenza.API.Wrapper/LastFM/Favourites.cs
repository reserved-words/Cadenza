...
internal class Favourites : IFavourites
{
    ...
    {
        var url = _url.Build(ApiEndpoints.Favourite);
        await _http.Post(url, null, track);
    }

    public async Task<bool> IsFavourite(string artist, string title)
    {
        var url = _url.Build(ApiEndpoints.IsFavourite, ("artist", artist), ("title", title));
    ...
    {
        var url = _url.Build(ApiEndpoints.Unfavourite);
        await _http.Post(url, null, track);
    }
}
